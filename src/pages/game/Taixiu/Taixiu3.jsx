...
function Taixiu3() {
	...
	useEffect(() => {
		...
		axios.get(`https://server.best96tx.com/taixiu3/get`).then((res) => {
			setBet(res.data.data);
			setDulieunhap(new Date(res.data.data.createdAt));
			setStart(true);
		});
		axios
			.get(`https://server.best96tx.com/taixiu3/getallbet`, {})
			.then((res) => {
					...
					swal({
						...
						switch (value) {
							case "share":
								setShare(true);
								setMessage(
									"hethong__Tài xỉu 3p__" +
										profile.username +
										"__" +
										formData.money +
										"__" +
										GetNameChoose(formData.result, null, "Tài xỉu 3p")
								);
								break;
							default:
						}
