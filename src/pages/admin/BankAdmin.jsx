...
function BankAdmin() {
    ...
    useEffect(() => {
        if (load === true) {
            axios.get(`${process.env.REACT_APP_API_URL}/auth/getbank`, {}).then((res) => {
    ...
    const handleSubmit = (e) => {
        ...
        axios
            .post(`${process.env.REACT_APP_API_URL}/auth/newbank`, formData)
            .then((res) => {
                setLoad(true);
                swal("Tạo ngân hàng thành công!").then((value) => {
    ...
    const handleSubmitBank = (e) => {
        ...
        axios.post(`${process.env.REACT_APP_API_URL}/auth/editlistbank`, formData).then((res) => {
                ...
                <DashboardLayout>
                    {
                        <Box
                            ...
                            <Container maxWidth={false}>
                                <div style={{ fontSize: "25px", fontWeight: 700 }}>Cài đặt ngân hàng</div>
                                <div style={{ marginTop: "20px" }}>
                                    <form onSubmit={handleSubmit} className="form-admin">
                                        <div>
                                            <div>
                                                <label>Tên người nhận</label>
                                                <input required name="fullname" step="any" id="fullname" />
                                            </div>
                                        </div>
                                        <div>
                                            <div>
                                                <label>Tên ngân hàng</label>
                                                <input name="name_bank" step="any" id="name_bank" required />
                                            </div>
                                        </div>
                                        <div>
                                            <div>
                                                <label>Số tài khoản</label>
                                                <input type="number" name="stk" step="any" id="stk" required />
                                            </div>
                                        </div>
                                        <div>
                                            <div>
                                            ...
                                            <>
                                                {bank.map((item) => (
                                                    <TableRow>
                                                        ...
                                                        <TableCell sx={{ fontWeight: "600", padding: "10px" }}>{item.title}</TableCell>
                                                        <TableCell sx={{ fontWeight: "600", padding: "10px" }}>
                                                            <Button
                                                                onClick={() => {
                                                                    axios
                                                                        .post(`${process.env.REACT_APP_API_URL}/auth/editlistbank`, {
                                                                            id: item._id,
                                                                            isShow: !item.isShow
                                                                        })
                                                            ...
                                                            <Button
                                                                onClick={() => {
                                                                    axios
                                                                        .get(`${process.env.REACT_APP_API_URL}/auth/bankall/${item._id}`, {})
                                                                        .then((res) => {
