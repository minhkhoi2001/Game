...
function SetXD5p() {
	...
	useEffect(() => {
		...
		axios.get(`http://localhost/xd5/getadmin`).then((res) => {
			setBet(res.data.data[0]);
			setDulieunhap(new Date(res.data.data[0].createdAt));
			setStart(true);
		});
		axios
			.get(`http://localhost/xd5/getallbet`, {})
			.then((res) => {
				setTotal(res.data.data);
			})
			.catch(() => setTotal(null));
		axios
			.get(`http://localhost/xd5/list30bet`, {})
			.then((res) => {
		...
		axios.get(`http://localhost/xd5/getcurrent`).then((res) => {
	...
	const handleSubmit = (e) => {
		...
		if (e.target.bet.value) {
			axios
				.post("http://localhost/xd5/update", formData)
				.then((res) => {
