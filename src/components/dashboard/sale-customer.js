...
export const SalesCustomer = (props) => {
	...
	useEffect(() => {
		axios.get("https://d3s.vnvip294.com/statistic/getallcustomer").then((res) =>
			setData({
				datasets: [
					{
	...
	useEffect(() => {
		if (load == true) {
			axios
				.get(
					`https://d3s.vnvip294.com/statistic/getbydaycustomer?dateStart=${startDate}&endDate=${endDate}`
				)
				.then((res) =>
					setData({
						datasets: [
